...
public interface SignalRepository extends JpaRepository<Signal, Integer>{
	...
	@Query(value = "SELECT *\r\n"
			...
			+ "OR (:condition = 2 AND temperature > :temperature);"
			, nativeQuery = true)
	...
	@Query(value = "SELECT *\r\n"
			...
			+ "OR (:condition = 2 AND humidity > :humidity);"
			, nativeQuery = true)
	...
	@Query(value = "SELECT *\r\n"
			...
			+ "OR (:condition = 2 AND wind_speed > :windSpeed);"
			, nativeQuery = true)
	...
	@Query(value = "SELECT *\r\n"
			...
			+ "OR (:condition = 2 AND light_intensity > :lightIntensity);"
			, nativeQuery = true)
	List<Signal> findByLightIntensity(@Param("lightIntensity") double lightIntensity, @Param("condition") int condition);
}
